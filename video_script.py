...
def add_subtitles(self) -> VideoFileClip:
    ...
    font_path = (
        self.font_file.path
        if self.font_file
        else os.path.join(os.getcwd(), "data", "Montserrat-SemiBold.ttf")
    )
    ...
    subtitle_box_color = self.subtitle_box_color or (1, 0.3, 0.5)
...
def concatenate_clips(self, clips):
    ...
    target_resolution = MAINRESOLUTIONS.get(
        self.resolution, (1920, 1080)
    )  # Default to '16:9' if resolution not found
    target_fps = self.fps
...
def generate_blank_video_with_audio(
    ...
    if not self.audio_file or not self.srt_file:
        raise ValueError(
            "Audio file and SRT file must be provided before generating the video."
        )
...
def generate_srt_file(self):
    ...
    output_srt_file_path = os.path.join(
        settings.MEDIA_ROOT, "srt_files", f"{self.pk}_output.srt"
    )
...
def resize_to_aspect_ratio(self, video_path: str) -> VideoFileClip:
    ...
    desired_aspect_ratio = MAINRESOLUTIONS.get(
...
def get_video_duration_from_srt(
    self,
):
    """
...
def convert_text_to_speech(self):
    ...
    try:
        ...
        audio_temp_path = os.path.join(
            settings.MEDIA_ROOT,
            "audio_files",
            f"{self.text_file.name}_{self.pk}_output.mp3",
        )
